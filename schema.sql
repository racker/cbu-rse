CREATE DATABASE IF NOT EXISTS `pollcat`
  CHARACTER SET = utf8;
...
create user 'pollcat'@'%' identified by 'insert-password-here';

-- Only for slave
grant select, execute on pollcat.* to 'pollcat'@'%';

-- Only for master
grant insert on pollcat.* to 'pollcat'@'%';
