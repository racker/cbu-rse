    ...
    def _create_report(self, profile_db, validate_db):
        ...
        for retry_counter in xrange(10):
            try:
                ...
                if max_event:
                    collstats_max_event_info[
                        'created_at'] = format_datetime(
                            max_event['created_at']
                        )
                    collstats_max_event_info['age'] = (
                        datetime.datetime.utcnow() - max_event['created_at']
                    ).seconds
                ...
                if min_event:
                    collstats_min_event_info['created_at'] = (
                        format_datetime(min_event['created_at'])
                    )
                    collstats_min_event_info['age'] = (
                        datetime.datetime.utcnow() - min_event['created_at']
                    ).seconds
        ...
        retry_rate = (
            0
            if (self.shared.id_totalcnt == 0 or self.shared.id_retrycnt == 0)
            else float(self.shared.id_retrycnt) / self.shared.id_totalcnt
        )
        ...
        mongo_stats = {
            "background_flushing": {
                "last_finished": str(
                    dbstats['backgroundFlushing']['last_finished']
                ),
            ...
            "cursors": {
                "client_cursors_size": (
                    dbstats['cursors']['clientCursors_size']
                ),
            ...
            "global_lock": {
                ...
                "current_queue": {
                    ...
                    "writers": (
                        dbstats['globalLock']['currentQueue']['writers']
                    ),
                    "readers": dbstats['globalLock']['currentQueue']['readers']
                },
                "lockTime": dbstats['globalLock']['lockTime'],
                "ratio": (
                    dbstats['globalLock']['lockTime'] /
                    dbstats['globalLock']['totalTime']
                ),
                "active_clients": {
                    "total": dbstats['globalLock']['activeClients']['total'],
                    "writers": (
                        dbstats['globalLock']['activeClients']['writers']
                    ),
                    "readers": (
                        dbstats['globalLock']['activeClients']['readers']
                    )
                }
            },
            "local_time": str(dbstats['localTime'])
        }
        mongo_coll_event_stats = {
            ...
            "avg_obj_size": (
                0
                if collstats_events['count'] == 0
                else collstats_events['avgObjSize']
            ),
        ...
        return json.dumps(
            {
                ...
                "mongodb": {
                    "stats": mongo_stats,
                    "coll_events_stats": mongo_coll_event_stats,
                    "online": db_online,
                    "error": db_error_message,
                    "database": self.mongo_db.name,
                    "profiling":  {
                        "reponse_length": str(
                            profile_info['responseLength']
                        ) if profile_db else "N/A",
                        "nreturned": str(
                            profile_info['nreturned']
                        ) if profile_db else "N/A",
                        "nscanned": str(
                            profile_info['nscanned']
                        ) if profile_db else "N/A",
                    },
                    "collection": {
                        "name": "events",
                    ...
                    "read_preference": (
                        self.mongo_db_connection.read_preference
                    ),
